...
@pytest.mark.parametrize(
    ...
    [
        ["not-an-object"],
        [{"actorSeatId": "seat-1", "rngStepBefore": 0, "rngStepAfter": 1}],
        [{"action": {}, "actorSeatId": "seat-1", "rngStepBefore": 0, "rngStepAfter": 1}],
        [{"action": {"type": "PASS"}, "rngStepBefore": 0, "rngStepAfter": 1}],
        [
            {
                "action": {"type": "PASS"},
                "actorSeatId": "",
...
def test_parity_invalid_history_entry_shape(history):
    envelope = {**_BASE, "history": history}
    assert eligibility_skip_reason(envelope) == "invalid_history"


def test_parity_missing_version_matches_web_invalid_replay():
    body = {k: v for k, v in _BASE.items() if k != "version"}
    assert eligibility_skip_reason(body) == "unsupported_version"


def test_parity_bool_version_rejected():
    assert eligibility_skip_reason({**_BASE, "version": True}) == "unsupported_version"
